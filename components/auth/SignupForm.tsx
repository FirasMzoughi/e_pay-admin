...
export function SignupForm() {
  ...
  const onSubmit = async (data: SignupFormValues) => {
    ...
    try {
      ...
      // Upon successful signup, user might need verification if email confirm is on.
      // Assuming straightforward signup for now or auto-login.
      // The middleware will handle redirection if session is established.
    ...
    <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-2xl shadow-xl border border-slate-100">
      <div className="space-y-2 text-center">
        ...
        <p className="text-slate-500">
          Sign up for E-Pay Admin Dashboard
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              disabled={isLoading}
              className={cn("bg-white", errors.fullName && "border-red-500")}
              {...register('fullName')}
            />
            {errors.fullName && (
